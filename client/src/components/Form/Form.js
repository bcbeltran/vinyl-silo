	...
	return (
		<Paper className={classes.paper}>
			<form
				autoComplete="off"
				noValidate
				className={`${classes.root} ${classes.form} ${classes.textfield}`}
				onSubmit={handleSubmit}
			>
				<Typography variant="h6">
					{currentId ? "Edit" : "Add"} an Album
				</Typography>
				<TextField
					name="title"
					variant="outlined"
				...
				<TextField
					...
					onChange={(e) =>
						setAlbumData({ ...albumData, year: e.target.value })
					}
				/>
				<TextField
					name="message"
					...
					onChange={(e) =>
						setAlbumData({ ...albumData, message: e.target.value })
					}
				/>
				<TextField
					name="tags"
				...
				<div className={classes.fileInput}>
					<FileBase
						...
						multiple={false}
						onDone={({ base64 }) =>
							setAlbumData({ ...albumData, selectedFile: base64 })
						}
					/>
				</div>
				<Button
